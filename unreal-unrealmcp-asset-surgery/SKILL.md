...
- 项目含 UnrealMCP 插件且编辑器在运行：`cmd /c start "" "<UE>/UnrealEditor.exe" "<project>.uproject" -nop4`
...
`UnrealEditor-Cmd.exe <uproject> -run=pythonscript -script=<py> -unattended -nop4 -nullrhi`：
...
- delete_asset 有引用失败；类已删的孤儿资产 load 失败 → 直接 rm 磁盘 .uasset；重命名残留的 ~2KB 重定向器同样 rm
